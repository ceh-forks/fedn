...
We do not provide an out-of-the box way to clear the compute package for a model that has been initialized. 
...
Once the federated model has been initialized and seeded it should be seen as immutable. However, during development of a new model
...
There are also additional ways to enable interactive development by bypassing the need to use/upload a compute package.  
...
https://github.com/scaleoutsystems/fedn/blob/master/fedn/fedn/aggregators/fedavg.py 
...
In the main control implementation https://github.com/scaleoutsystems/fedn/blob/master/fedn/fedn/clients/reducer/control.py you can modify or replace the method "check_round_validity_policy". As we expand with more implementations of this policy, we plan to make it runtime configurable. 

Q: Can I start a client listening only to training requests or only on validation requests?:
-------------------------------------------------

Yes! From FEDn 0.3.0 there is an option to toggle which message streams a client subscribes to. For example, to start a pure validation client: 
...
Yes. We plan to add additional security mechanisms, such as differential privacy and multiparty computation (between client-combiner or for the reduce protocol). Please let us know what you need in your projects to help us prioritize development.   
